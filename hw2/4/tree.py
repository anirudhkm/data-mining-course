...
def split_data(df, feature, impurity_dict, tree_dict,
              curr_node_dict, impurity_measure = "gini"):
    """
    ...
    decision_tree(low_data_df, tree_dict, curr_node_dict,
                  feature, align_str = 'Left', impurity_measure = impurity_measure)
    # function call
    ...
    decision_tree(high_data_df, tree_dict, curr_node_dict,
                  feature, align_str = 'Right', impurity_measure = impurity_measure)
    # function call
...
def display_tree(tree, inp_file = '', tab_space = 0, header = False):
    ...
    if header:
        # print the header
        print colored("{}".format(inp_file),attrs = ['bold', 'underline'])
    for i in iter(tree):
        # iterate through the tree
        print tab_space*'\t', colored(i, "red")
        if isinstance(tree[i]["Right"], set):
            print (tab_space+1)*'\t', colored(tree[i]["Right"], "green")
        else:
            display_tree(tree[i]["Right"], tab_space = tab_space + 1)

        if isinstance(tree[i]["Left"], set):
            print (tab_space+1)*'\t', colored(tree[i]["Left"], "green")
        else:
...
def main(input_file, impurity_measure):
    ...
    tree_dict = {}
    # empty dict on which tree will be built
    decision_tree(df, tree_dict, {},
                 impurity_measure = impurity_measure)
        # function call to construct the tree
    display_tree(tree_dict, input_file, header = True)
    # function call to display tree
